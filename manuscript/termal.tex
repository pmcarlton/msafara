...
\keywords{multiple sequence alignment, visualisation, terminal, text user interface}
...
example \texttt{alan}  \citep{alan}, which stands out as a particularly elegant solution,
...
\texttt{showalign}  \citep{emboss} can compute a consensus, it does not support
colouring residues, and the user must explicitly call a pager in order to scroll
through the alignment. Other programs like \texttt{alen}  \citep{alen} are
...
Jalview's  \citep{waterhouse2009jalview} nucleotide colour scheme, while protein
alignments use that of ClustalX  \citep{larkin2007clustal}. An alternative colour
scheme for protein is Lesk's  \citep{lesk2019introduction}, and all alignments can
...
\texttt{termal} and \texttt{alen} are written in Rust  \citep{klabnik2019rust} and
compile to a single binary with no dependencies. By contrast,
\texttt{alv}  \citep{Arvestad2018} requires BioPython and \texttt{showalign} is
...
Alacritty  \citep{alacritty}, Kitty  \citep{kitty} and Ghostty  \citep{ghostty} offer
...
This work was supported by the Swiss National Science Foundation BRIDGE Discovery
funding scheme [grant 40B2-0\_194701]. The author wishes to thank Drs Guillaume
Cailleau and Sébastien Moretti for insightful comments on the program.
...
\bibliographystyle{mynat} % doesn't seem to work
